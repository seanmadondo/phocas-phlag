import { ISearchContext, ISearchTerm, ISearchTermProvider, IStorageProvider, IStorageCache, SearchTermLoadType } from "./types";
...
export default class StorageCache implements IStorageCache {
	...
	constructor(storageProvider: IStorageProvider, searchTermProvider: ISearchTermProvider, ttlSeconds: number) {
		this.storageProvider = storageProvider;
		this.searchTermProvider = searchTermProvider;
		this.ttlSeconds = ttlSeconds;
	}

	private static async getStoredSearchTerm(searchTerm: ISearchTerm): Promise<IStoredSearchTerm> {
	...
	private static getSearchTerm(storedSearchTerm: IStoredSearchTerm): ISearchTerm {
		return SearchTerm.fromUrl(storedSearchTerm.value, storedSearchTerm.type, SearchTermLoadType.Dynamic, storedSearchTerm.url);
	}
	...
	private async getDynamicSearchTerms(forceRefresh: boolean): Promise<ISearchTerm[]> {
	...
	private async setDynamicSearchTerms(searchTerms: ISearchTerm[]) {
		const storedSearchTerms: IStoredSearchTerm[] = await Promise.all(searchTerms.map(StorageCache.getStoredSearchTerm));
	...
	async clear() {
		await Promise.all(KEYS.map(async (key) => await this.storageProvider.reset(key)));
	}
}