...
class CompositeSearchTermProvider implements ISearchTermProvider {
	...
	loadStaticTerms = async () => {
		return (await Promise.all(this.providers.map((provider) => provider.loadStaticTerms()))).flat();
	}

	loadDynamicTerms = async () => {
		return (await Promise.all(this.providers.map((provider) => provider.loadDynamicTerms()))).flat();
	}
